...
MSG_id=$((${message_message_id} + 1 ))
	...
	local bot_retorno="  💎𝙱𝚒𝚎𝚗𝚟𝚎𝚗𝚒𝚍𝚘  𝚊𝚕  𝙱𝚘𝚝𝙶𝚎𝚗  𝙰𝙳𝙼 💎 \n"
		bot_retorno+="$LINE\n"
        bot_retorno+=" ❌| Sin acceso al bot. |❌\n ✨Contacta a @drowkid01 para adquirir una suscripción de este bot. ✨\n"
        bot_retorno+=" 😎 El comando /keygen fue habilitado por @drowkid01 para todos😎\n | 🔑| Puedes generar 3 keys multi al día ó puedes contactar a @drowkid01 para una key|🔑"
        bot_retorno+="🔓| La key multi es una key capaz de instalar otros scripts conocidos como el de ChumoGH, LATAM y rufus (proximamente)🔓| ÚSALA!\n"
        bot_retorno+="$LINE\n"
	    ShellBot.sendMessage --chat_id $var \
							--text "<i>$(echo -e $bot_retorno)</i>" \
							--parse_mode html
...
send_admin(){
	...
	bot_retorno2+="📌Solicitud de acceso📌\n"
	bot_retorno2+="$LINE\n"
	bot_retorno2+="<u>name</u>: ${callback_query_from_first_name}\n"
	[[ ! -z ${callback_query_from_username} ]] && bot_retorno2+="<u>user</u>: @${callback_query_from_username}\n"
	bot_retorno2+="<u>id</u>: <code>${callback_query_from_id}</code>\n"
	bot_retorno2+="$LINE"

	bot_retorno="$LINE\n"
	bot_retorno+="     🚀| ＤｒｏｗｋＢｏｔ | 🚀\n"
	bot_retorno+="           😎@drowkid01😎\n"
	bot_retorno+="$LINE\n"
	bot_retorno+="      ✅ Solicitud de acceso enviada✅\n"
	bot_retorno+="$LINE"
	comand_boton "Ok"
...
ShellBot.InlineKeyboardButton --button 'botao_user' --line 1 --text '🔑KEY SPACE🔑' --callback_data '/keygen'
